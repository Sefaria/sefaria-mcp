...
| `SEFARIA_MCP_FIRST_PARTY` | unset | Set to `1` **only on Sefaria's own deployment**. Outbound calls to the Sefaria API then carry the User-Agent `Sefaria/sefaria-mcp` instead of the default `sefaria-mcp`, so Sefaria can tell its own server apart from self-hosted copies in its logs |
